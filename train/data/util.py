...
class bFFHQDataset(Dataset):   
    def __init__(self,
        ...
        if split=='train':
            ...
            if include_generated:
                self.generated = glob(os.path.join(preproc_root, 'align', '*', 'imgs', '*'))
                self.data += self.generated

        elif split=='valid':
            self.data = glob(os.path.join(os.path.dirname(root), split, '*'))

        elif split=='test':
            if self.args.test_on_biased:
                print("** Testing on biased testset **")
                self.data = glob(os.path.join(os.path.dirname(root), split, '*'))
                data_conflict = []
...
def get_dataset(args,
    ...
    if args.dataset == 'cmnist':
        ...
        dataset = CMNISTDataset(root=root, 
                                split=dataset_split,
                                transform=transform,
        ...
        print('Wrong dataset ...')
        print('Select one of them: cmnist, bffhq, bar, dogs_and_cats, cifar10c')
        import sys
        sys.exit(0)
