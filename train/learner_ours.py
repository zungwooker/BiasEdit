...
class LearnerOurs(object):
    def __init__(self, args):
        ...
        self.train_dataset_b = get_dataset( # Given biased dataset: D_b
            args=args,
            dataset_split="train",
        ...
        self.train_dataset_d = get_dataset( # Debiased dataset(D_b + D_g): D_d
            args=args,
            dataset_split="train",
            transform_split="train",
            use_preprocess=self.preprocess,
        ...
        # For EMA of computing relative difficulty score
        train_target_attr = []
        for data in self.train_dataset_d.data:
        ...
        self.train_dataset_b = IdxDataset(self.train_dataset_b)
        self.train_dataset_d = IdxDataset(self.train_dataset_d)
        ...
        self.train_loader_d = DataLoader(
            self.train_dataset_d,
            batch_size=self.batch_size,
            shuffle=True,
            num_workers=args.num_workers,
            drop_last=True
        ...
        self.pretrain_loader = DataLoader( # The loader that BCDs use for voting. NOT FOR TRAINING
            self.train_dataset_d, # vote for D_d
            batch_size=self.batch_size,
            shuffle=False,
            num_workers=args.num_workers,
            drop_last=False
    ...
    def pretrain_b_ensemble_best(self, args):
        train_iter = iter(self.train_loader_b)
    ...
    def train_vanilla(self, args): 
        ...
        train_iter = iter(self.train_loader_d)
    ...
    def train_lff(self, args):
        ...
        train_iter_b = iter(self.train_loader_b)
        ...
        for step in tqdm(range(args.num_steps)):
            ...
            # Load a batch from D_d
            try:
                index_d, data_d, attr_d, _ = next(train_iter_d)
            except:
                train_iter_d = iter(self.train_loader_d)
                index_d, data_d, attr_d, _ = next(train_iter_d)
                
            data_d = data_d.to(self.device)
            attr_d = attr_d.to(self.device)
            index_d = index_d.to(self.device)
            label_d = attr_d[:, args.target_attr_idx]
            
            # Load a batch from D_b
            try:
                index_b, data_b, attr_b, _ = next(train_iter_b)
            except:
                train_iter_b = iter(self.train_loader_b)
                index_b, data_b, attr_b, _ = next(train_iter_b)
            ...
            # Compute relative difficulty score(W)
            with torch.no_grad():
                logit_b = self.model_b(data_d)
            logit_d = self.model_d(data_d)
            print("check no gradient in model_b")
            breakpoint()
            
            loss_b = self.criterion(logit_b, label_d).detach()
            loss_d = self.criterion(logit_d, label_d).detach()
            ...
            # EMA sample loss
            self.sample_loss_ema_b.update(loss_b, index_d)
            self.sample_loss_ema_d.update(loss_d, index_d)

            # class-wise normalize
            loss_b = self.sample_loss_ema_b.parameter[index_d].clone().detach()
            loss_d = self.sample_loss_ema_d.parameter[index_d].clone().detach()
            ...
            logit_b = self.model_b(data_b)
            loss_b_update = self.bias_criterion(logit_b, label_b)
            loss_d_update = self.criterion(logit_d, label_d) * loss_weight.to(self.device)
    ...
    def train_lff_be(self, args):
        ...
        train_iter_b = iter(self.train_loader_b)
        train_iter_d = iter(self.train_loader_d)
        train_num = len(self.train_dataset_b.dataset)
        ...
        for step in tqdm(range(args.num_steps)):
            ...
            logit_b = self.model_b(data_b)
            loss_b_update = self.criterion(logit_b[curr_align_flag], label_b[curr_align_flag])
            loss_d_update = self.criterion(logit_d, label_d) * loss_weight.to(self.device)

            if np.isnan(loss_b_update.mean().item()):
                loss_b_update = torch.tensor(0, dtype=torch.float32) # in this batch, no psuedo BA
